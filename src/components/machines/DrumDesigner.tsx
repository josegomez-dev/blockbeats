...
const DrumDesigner = () => {
  ...
  const [grid, setGrid] = useState<boolean[][]>(
    Array(INITIAL_INSTRUMENTS).fill(null).map(() => Array(16).fill(false))
  );
  const [selectedSounds, setSelectedSounds] = useState<string[]>(
    DRUM_SOUNDS.slice(0, INITIAL_INSTRUMENTS).map((sound) => sound.name)
  );
    ...
    <div className={styles.drumDesigner}>
      ...
      <div className={styles.grid}>
        {grid.map((row, rowIndex) => {
          const currentType = getTypeOfSelected(rowIndex);
          const options = currentType
            ? DRUM_SOUNDS.filter((s) => s.type === currentType)
            : DRUM_SOUNDS;
          ...
          return (
            <div key={rowIndex} className={styles.row}>
              ...
              >
                <span className={styles.iconOnly}>{currentSound?.icon}</span>
